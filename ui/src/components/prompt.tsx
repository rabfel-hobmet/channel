...
export default function Prompt() {
    ...
    const dismissPrompt = () => {
        setPrompt(false);
        return localStorage.setItem('channel-prompt-0.0.3', JSON.stringify(false))
      }
    useEffect(() => {
        const prompt = Boolean(JSON.parse(localStorage.getItem('channel-prompt-0.0.3') ?? "true"));
        if (prompt === false) {
          setPrompt(prompt)
        }
      }, []);
    ...
    <div className="bg-chan-element w-full max-w-prose max-h-96 overflow-auto border-2 border-chan-border p-4 flex flex-col space-y-4 text-sm">
      ...
      <p><span className="text-chan-red font-bold">Welcome to channel 0.0.3</span>, Urbit's premier imageboard application. We are in closed beta. To make the most of your experience using this software, please note the following:</p>
    ...
    <li>Greentext</li>
    <li>Bluetext</li>
    <li>Postboxes are at top</li>
    <li>Fix homepage requiring refreshes to show boards</li>
    <li>Adds %docs integration</li>
    <p>To come:</p>
